...
public class EnemyLevel4 : MonoBehaviour
{
	...
	public int stage, countSpawn4, randomNumber;
  ...
  public int dropChance;
    ...
    void Start()
    {
	   ...
	   if(gameObject.transform.GetChild(1).name==("Hand4")){
		   health = stage*15 + 500;
	   }
	...
	if(GameObject.FindGameObjectWithTag("Player")!=null){ 
		...
		speedMove = 0.9f+(stage*0.1f) + 1.5f;
			rb.mass = 0;
		...
		else if (playerDistance<3 && playerDistance>1.5 && randomNumber<2){
	...
	if(GameObject.FindGameObjectWithTag("turret")!=null){
	  ...
	  if(GameObject.FindGameObjectWithTag("Player")==null){
	   ...
	   float angle  = Mathf.Atan2(direction.y, direction.x) * Mathf.Rad2Deg;
		...
		if (animatorStateInfo.IsName("zombie4Death")){
			time -=Time.deltaTime;
			
			if(time < 0){
		isBleeding = false;
		isAttacking = false;		
		//Instantiate(deadPref, new Vector3(gameObject.transform.position.x,gameObject.transform.position.y-0.83f, gameObject.transform.position.z) , Quaternion.Euler(new Vector3(0 , 0, gameObject.transform.rotation.eulerAngles.z)));
